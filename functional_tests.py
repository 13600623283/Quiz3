...
class NewVisitorTest(unittest.TestCase):
	...
	# here is the first test (for free) 
        ...
        self.assertIn('Convergence of Rationalism and Emotionalism',self.browser.title)
        m=self.browser.find_element_by_tag_name('img')
        self.assertIn('principle.jpg',m.get_attribute('src'))
        h=self.browser.find_element_by_css_selector('h1')
        self.assertIn("Convergence of Rationalism and Emotionalism",h.text)
        a=self.browser.find_element_by_id('newpage')
        a.click()
        l=self.browser.current_url
        self.assertIn('newpage.html', l)
